  ...
  return (
    ...
    >
      <div className="flex items-center justify-between">
        <span className="font-mono text-eyebrow tracking-wide text-ink-3">{code}</span>
      ...
      <div className="flex items-baseline justify-between gap-2">
        <span className="text-body font-semibold leading-tight text-ink">{level.label}</span>
        {level.comprehension != null && (
          <span className="shrink-0 font-mono text-micro text-ink-3 tnum" title="Comprensión ISO 9186-1 (ilustrativa)">
            {level.comprehension}%
          </span>
        )}
      </div>
      <span className="mt-0.5 text-eyebrow leading-snug text-ink-3">{cat.name} · {level.short}</span>
    </button>
  );
}
